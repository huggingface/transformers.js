...
| `early_stopping`? | `boolean\|"never"` | Controls the stopping condition for beam-based methods, like beam-search. It accepts the following values:<br />- `true`, where the generation stops as soon as there are `num_beams` complete candidates;<br />- `false`, where an heuristic is applied and the generation stops when is it very unlikely to find better candidates;<br />- `"never"`, where the beam search procedure only stops when there cannot be better candidates (canonical beam search algorithm). _(default: `false`)_ |
...
| `encoder_repetition_penalty`? | `number` | The parameter for encoder_repetition_penalty. An exponential penalty on sequences that are not in the original input. 1.0 means no penalty. _(default: `1.0`)_ |
