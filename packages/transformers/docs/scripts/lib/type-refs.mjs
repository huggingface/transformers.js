...
const RENDERABLE_UTILITY_NAMES = [
  UTILITY_TYPES.PARAMETERS,
  UTILITY_TYPES.RETURN_TYPE,
  UTILITY_TYPES.CONSTRUCTOR_PARAMETERS,
];
const RENDERABLE_UTILITY_PATTERN = new RegExp(
  `^(${RENDERABLE_UTILITY_NAMES.join("|")})<(.+)>(?:\\[(\\d+)\\])?$`,
);
