  ...
  constructor(...args) {
    if (isONNXTensor(args[0])) {
      ...
      this.ort_tensor = new ONNXTensor(
        ...
        // @ts-ignore - ts(2769): Type 'number' is not assignable to type 'bigint' but works at runtime
 ...
 */
export async function interpolate_4d(
  input,
  { size = null, mode = "bilinear" } = {},
) {
  // Error checking
  ...
  const sizeTensor = new Tensor(
    "int64",
    new BigInt64Array(targetDims.map(BigInt)),
    [targetDims.length],
  );
...
const arrayToIndexTensor = (array) =>
  new Tensor("int64", array, [array.length]);
/**
  ...
  for (let i = 0; i < batchSize; ++i) {
    ...
    for (let j = 0; j < featureDim; ++j) {
      ...
      returnedData[offset2] =
        (inputData[offset2] - meanData[i]) / (stdData[i] + eps);
    }
  }
...
export function std_mean(input, dim = null, correction = 1, keepdim = false) {
  ...
  if (dim === null) {
    ...
    const std = Math.sqrt(
      inputData.reduce((a, b) => a + (b - mean) ** 2, 0) /
