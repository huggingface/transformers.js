...
export class Glm46VProcessor extends Processor {
    ...
    async _call(text, images = null, ...args) {
        ...
        if (image_grid_thw) {
            ...
            text = text.map((t) => {
                while (t.includes('<|image|>')) {
                    const prod = Number(image_grid_thw_list[index++].reduce((a, b) => a * b, 1n));
                    t = t.replace('<|image|>', '<|placeholder|>'.repeat(Math.floor(prod / merge_length)));
                }
                return t.replaceAll('<|placeholder|>', '<|image|>');
            });
        }
