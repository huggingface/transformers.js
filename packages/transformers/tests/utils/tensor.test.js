...
describe("Tensor operations", () => {
  ...
  describe("cat", () => {
    it("should concatenate on dim=0", () => {
      ...
      expect(concatenated1).toEqual(target1);

      // 3 tensors
      const concatenated2 = cat([t1, t2, t3], 0);
      expect(concatenated2).toEqual(target2);
    });
    ...
    it("should concatenate on dim=-2", () => {
      ...
      expect(concatenated).toEqual(target);
    });
  ...
  describe("stack", () => {
    ...
    it("should stack on dim=0", () => {
      ...
      expect(stacked1).toEqual(target1);

      // 3 tensors
      const stacked2 = stack([t1, t1, t1], 0);
      expect(stacked2).toEqual(target2);
    });
  ...
  describe("permute", () => {
    it("should permute", () => {
      ...
      const target_1 = x;
      expect(permuted_1).toEqual(target_1);
      ...
      expect(permuted_2).toEqual(target_2);
      ...
      expect(permuted_3).toEqual(target_3);
      ...
      expect(permuted_4).toEqual(target_4);
      ...
      expect(permuted_5).toEqual(target_5);
      ...
      expect(permuted_6).toEqual(target_6);
    });
  });
  ...
  describe("std_mean", () => {
    it("should return std_mean for the entire tensor", () => {
      ...
      expect(stdVal).toBeCloseToNested(new Tensor("float32", [1.8708287477493286], []));
      expect(meanVal).toBeCloseToNested(new Tensor("float32", [3.5], []));
    });
  });
  ...
  describe("normalize", () => {
    it("should normalize a vector correctly", () => {
      ...
      expect(normalized).toBeCloseToNested(target);
    });
  ...
  describe("layer_norm", () => {
    it("should calculate layer norm", () => {
      ...
      expect(norm).toBeCloseToNested(target);
    });
  });
